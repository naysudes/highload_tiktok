...
Установим сервера в Москву, Петербург и Екатеринбург.

Узким местом данного решения является прогон видео через бекенд узлы. Попробуем расширить его. Для того, чтобы при пиковой нагрузке добиться пропускной способности в 21.6 Тб/с, нам необходимо 21600 / (50 * 2) = 216 серверов, для надежности, возьмем 250 серверов. На все сервера поставим две сетевые карты по 50 Гб/с. Все машины будет иметь 64 CPU.

Таким образом, мы сможем прогнать весь трафик через бекенд-ноды.
...
Узким местом данного решения является прогон видео через бекенд узлы. Попробуем расширить его. Для того, чтобы при пиковой нагрузке добиться пропускной способности в 21.6 Тб/с, нам необходимо 21600 / (50 * 2) = 216 серверов, для надежности, возьмем 250 серверов. На все сервера поставим две сетевые карты по 50 Гб/с.


## 7.3 Likes&Comments Backend

Будем отталкаваиться от максимальной пиковой нагрузки - 2727. Конфигурация идентична Video-watching backend, но не требует такого кол-ва сетевых карт и машин. Поставим 3 основных сервера и 3 дублера.
...
| Video-watching backend|   64   | 16 |   128    |  64         | 2 х 10 Гб/с      | 250    | Москва, Петербург Екатеринбург |
| Video-uploading backend|   64   |16  |   128    |  64         | 2 х 10 Гб/с      | 250    | Москва, Петербург, Екатеринбург |
...
Узкое место данного решения - большая сетевая нагрузка на бекенд-сервера, отвечающие за просмотр и загрузку видео. Мы смогли расширить его за счет увеличения кол-ва этих нод, но данный подход неоптимален. В данной главе я предлагаю альтернативное решение, в котором пользователь не гонит трафик через бекенд узлы системы, а связывается с CDN напрямую.

Расчеты по нагрузке остаются те же, что и в основном решении. Redis, PostgreSQL - функционал использования и характеристики так же остаются неизменны.

## 9.1. Схема проекта
![alt text](alt_system_scheme.png "Схема проекта")

В данном случае, пользователь будет взаимодействовать с Video-watching backend для того, чтобы тот предоставил ему список идентификаторов видео, подготовленных системой рекомендаций. После этого, пользователь обращается к CDN для получения видео. Данное взаимодействие происходит без посредников, что позволяет сэкономить на сетевых ресурсах.

При загрузке видео пользователь отсылает файл в Video-uploading server, который сохраняет его на свой диск. Данный процесс происходит так же без посредников. После сохранения, новое видео загружается в несколько холодных CDN нод, например в каждую сотую ноду, т.е. в 3 ноды. Определить конкретный идентификтор нод можно методом взятия хеша от названия + времени загрузки видео, после чего взять остаток от деления на 100. Отправив видео в CDN, сервер заносит новую информацию в PostrgeSQL.

В остальном принцип работы аналогичен основному решению.

## 9.2. Расчет оборудования и расположения

### 9.2.1 Video-watching backend

Пиковая нагрузка на получение списка рекомендаций 2649 rps. Допустим, что бэкенд-сервер на голанг имеет производительность 4000 rps.
Установим сервера в Москву, Петербург и Екатеринбург.

Так же установим в том же географическом месте по дублеру данных бекендов.


### 9.2.2 Video-uploading server

Узким местом данного сервиса является прогон видео.
В месяц в систему загружается 15 млн видео, следовтельно в секунду в систему загружается 15000000 / ( 60 * 60 * 24 * 30) = 5.8 видео. Округлим до 6 видео в секунду. Допустим, что пиковая нагрузка будет равна удвоенной средней. Т.е. 12 видео в секунду.

Средний вес каждого видео - 68 Мб. Таким образом, пропускная способность данного сервиса должна быть равна (12 * 68) * 8 / 100 = 65.28 Гб/с.

Так же сервер должен отправлять видео в CDN. Учитывая это, возьмем сервер с материнкой на 4 PCI слота, чтобы разместить 4 сетевые карты на 50 Гб/с.

Для надежности, а так же для меньшей задержки запроса, стоит взять 10 машин.


### 9.2.3 Likes&Comments Backend

Будем отталкаваиться от максимальной пиковой нагрузки - 2727. Конфигурация идентична Video-watching backend. Поставим 3 основных сервера и 3 дублера.


### 9.2.4 Recommendation system
...
| Video-uploading server           |   64  | 16  |  128     |  64         | 4 х 50 Гб/с    | 10           | Москва, Петербург, Екатеринбург |
| Likes&Comments Backend           |   4   | 16  |  128     |  64         | 2 х 10 Гб/с    | 3 + 3        | Москва, Петербург, Екатеринбург |
| Recommendation system            |   32  | 128 |  128     |  64         | 2 х 10 Гб/с    | 5            | Москва, Петербург, Екатеринбург |
| PostgreSQL                       |   16  | 16  |  0       |  4000       | 2 х 10 Гб/с    | 1            | Москва                          |
| Модули CDN с популярным контентом|   8   | 16  |  6000    |  64         | 2 х 50 Гб/с    | 100          | Описано в 7.7                   |
...
Нам удалось сильно сократить количество требуемых серверов, что косвенно привело к многократной экономической выгоде альтернативного решения.
Добиться этого удалось за счет взаимодействия с сервисами загрузки/транслирования видео напрямую, а не через дополнительную бекенд-ноду.
