...
* Среднее количество комментариев - 15.
...
по ключу - id пользователя. Получается, что в Redis по id пользователя будем получать list, содержащий ссылки на рекомендованне видео. Информацию о 100 последних рекомендованных видео так же сохраним в БД (таблица UserFeeds)
...
Одна пара ключ-значение:
    id(16) + list(8) + 70 * video_link(80) = 5624 байта
Общий объем хранилища
    5624 * 30000000 = 168,72 Гбайт


Примерное расположение CDN-серверов:
![alt text](cdn_map.png "Схема проекта")
